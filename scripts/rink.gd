...
# Невидимое "стекло" над бортами: редкие рикошеты подбрасывают шайбу выше
# борта, а правил аута нет — шайба не должна покидать игру.
const GLASS_HEIGHT := 3.0
const GOAL_LINE_FROM_END := 4.0  # линия ворот в 4 м от торцевого борта
const MARK_Y := 0.02

const MARK_RED := Color(0.84, 0.16, 0.22)
const MARK_BLUE := Color(0.14, 0.30, 0.66)

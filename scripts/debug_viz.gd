...
func _physics_process(_delta: float) -> void:
	...
	# Сектор дуги клюшки: дуга по границе + граничные линии от точки хвата,
	# и текущее положение крюка (стрелка от pivot).
	if blade:
		var outline: PackedVector3Array = blade.zone_outline()
		var pivot: Vector3 = blade.pivot_point + Vector3(0.0, 0.04, 0.0)
		...
		for edge_point in [outline[0], outline[outline.size() - 1]]:
			_mesh.surface_set_color(COLOR_ZONE)
			_mesh.surface_add_vertex(pivot)
			_mesh.surface_set_color(COLOR_ZONE)
			_mesh.surface_add_vertex(edge_point)
		_add_arrow(pivot, blade.global_position - pivot, COLOR_BLADE_TARGET)
		# Луч фактического направления броска (висит 1 с после выстрела).
		if blade.aim_timer > 0.0:
			var af: Vector3 = blade.aim_from + Vector3(0, 0.05, 0)
			var at: Vector3 = blade.aim_to + Vector3(0, 0.05, 0)
			_mesh.surface_set_color(Color(1.0, 0.2, 0.1))
			_mesh.surface_add_vertex(af)
			_mesh.surface_set_color(Color(1.0, 0.2, 0.1))
			_mesh.surface_add_vertex(at)
	_mesh.surface_end()

	_label.global_position = pos + Vector3(0.0, 2.4, 0.0)
	var mode := "БЕГ"
	if _player.is_glide:
		mode = "СКОЛЬЖЕНИЕ"
	var txt := "%.1f м/с  [%s]%s%s%s%s" % [hvel.length(), mode,
			"  [СТОЙКА]" if _player.is_stance else "",
			"  [ЮЗ]" if _player.is_skidding else "",
			"  [СПРИНТ]" if _player.is_sprinting else "",
			"  [СПИНОЙ]" if _player.is_backward else ""]
	if blade:
		txt += "\nШАЙБА: %s  (%s)" % [blade.puck_state_name, blade.last_transition]
		if blade.puck_state == 1:  # CARRIED: сторона для правила подброса
			txt += "  [%s]" % ("ВНЕШНЯЯ" if blade.puck_is_external(_player.params) else "внутренняя")
		elif blade.puck_state == 2:  # AIR: окно набивания
			txt += "  набивание: %s" % ("ОКНО %d%%" % int(blade.juggle_frac * 100.0)
					if blade.juggle_open else "ждём окно")
		txt += "\nкрюк θ=%.0f° поза:%s(%d%%)%s" % [blade.azimuth, blade.pose_name,
				int(blade.pose_progress * 100.0),
				"  [ПЕРЕКЛАДКА→%s]" % ("хват" if blade.transfer_side > 0 else "неудобн.") if blade.transfer_active else ""]
		if blade._dribble_side != 0:
			txt += "  [ДРИБЛИНГ ширина %d%%]" % int(blade.dribble_width * 100.0)
		txt += "\nфинт: %s   Q/E: %s" % [blade.last_gesture, blade.qe_mode]
		if blade.poke_active:
			txt += "  [ОТБОР-выпад]"
		if blade._skate_feint_t > 0.0:
			txt += "  [конёк: протяжка %d%%]" % int((1.0 - blade._skate_feint_t / 0.3) * 100.0)
	# Стамина и готовность рывка — в HUD персонажа (hud.gd), не в F2.
	if _player.is_stunned():
		txt += "  [СТАН]"
	if _player.last_hit_foul:
		txt += "  [хит: ФОЛ]"
	_label.text = txt
